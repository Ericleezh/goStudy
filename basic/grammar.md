...
59. 核心处理器与协程的关系
    * 对于N个核心的情况设置GOMAXPROCS为n-1以获得最佳性能
    * 经验法则：`协程的数量 > 1 + GOMAXPROCS > 1`

60. 通道(channel：用于发送类型化数据的管道，由其负责协程间的通信，从而避开所有由共享内存导致的陷阱
    * 通过通道进行通信的方式保证**同步性**
    * 数据在通道中进行传递
    * 在任何给定时间里，一个数据被设计为只有一个协程可以对其访问，也就不会有数据竞争
    * **通道只能传输一种类型的数据**(可创建通道的通道类型)
    * 类型化消息的队列,FIFO结构可以保证元素的顺序性
    * 通道的发送和接收都是**原子**操作(互不干扰)

61. 通道阻塞：默认情况下，通信是同步且无缓冲的，在有接收者接收数据之前，发送不会结束，必须要一个接收者准备好接收通道的数据然后发送者可以直接把数据发送给接收者。所以**通道的发送/接收操作在对方准备好之前是阻塞的**：
...
62. 通道相关使用模式：
    * 通道工厂模式：通过函数生成一个通道并返回(工厂角色);函数内有匿名函数被协程调用
    ```go
    func main() {
        stream := pump()
        go suck(stream)
...
63. 通道的方向
    * `var recv_only chan<- int ` //只接受类型通道
    * `var send_only <-chan int` //只发送类型通道
    * 只接收的通道是无法关闭的，关闭通道指的是发送者不再发送值，所以对于只接收的通道没有意义

64. `select`切换协程(选择处理列出的多个通信情况中过的一个)
    * 若所有协程都阻塞了，则会等待到其中一个可以处理为止
    * 若多个可以处理，则会**随机选择**一个协程
    * 若没有通道操作可以处理并且写了`default`，则执行default的操作
    
65. netchan包：实现类型安全的网络化通道：它允许一个通道两端出现由网络连接的不同计算机。其实现原理是，在其中一台机器上将传输数据发送到通道中，那么就可以被另一台计算机上同类型的通道接收。一个导出器（exporter）会按名称发布（一组）通道。导入器（importer）连接到导出的机器，并按名称导入这些通道。之后，两台机器就可按通常的方式来使用通道。网络通道不是同步的，它们类似于带缓存的通道。
